...
[이전 챕터](https://www.fusetools.com/docs/tutorial/look-and-feel) 에서 우리는 앱의 룩앤필 (모양과 느낌) 에 초점을 맞췄고, 이제 멋지고 일관성있게 보이게 되었습니다. 이 과정에서 우리는 또한 Fuse의 `ux:Class` 메커니즘을 활용하여, 앱 스타일링을 위한 재사용 가능한 컴포넌트를 구축하는 방법을 학습 했습니다.

이 챕터에서는, 최종 화면 (스플래시 페이지)을 추가하고, 모든 것을 묶어 앱을 완성 할 것입니다! 이것으로 사용자가 앱을 열때 곧 우리 앱에 대한 분위기를 설정 것이고, 우리의 디자인과 일치시킬 수 있는 퍼즐의 마지막 부분입니다. 이전 챕터에서 배웠던 모든 도구들과 기술들을로 쉽게 할 수 있을 것입니다. 그럼 시작합시다!
...
우리가 마지막 챕터를 만들었던 `hikr.Page` 컴포넌트 대신, `Page` 를 기본 클래스로 사용하는 것에 주목하십시오. 결국 이 페이지에 비디오 배경을 추가 할 것이므로, 지금은 배경에 대한 앱의 배경색만을 원합니다.

이제 `SplashPage` 가 생겼으니, `MainView.ux` 의 앱 [Navigator](https://www.fusetools.com/docs/fuse/controls/navigator) 에 템플릿을 하나 추가해 봅시다. 또한 우리 앱에서 보여주고자 하는 첫 화면으로써, 이것을 기본 템플릿으로 만듭니다.:
...
이 `Grid` 의 첫 번째 하위 노드는, `Grid` 의 맨 위 셀의 내용을 나타냅니다. 우리의 제목 텍스트와 슬로건을 위해 [StackPanel](https://www.fusetools.com/docs/fuse/controls/stackpanel) 을 만듭니다.:
...
우리는 조만간 그것을 수직으로 가운데 위치시킬 것입니다. 지금은, 조금 더 `FontSize` 를 늘릴 것입니다 :
...
자, 우리의 슬로건 텍스트를 추가합시다. 제목 텍스트와 동일한 `StackPanel` 에 배치할 것입니다.:
...
또한 이 텍스트를 수평으로 가운데에 배치하고, `Opacity` 를 설정하여 텍스트를 투명하게 만듭니다.
...
그리고 그게 전부입니다; 우리의 제목과 슬로건을 살펴봤습니다!
...
아, 그런데 잠깐만요! `hikr.Button` 컴포넌트는 [Panel](https://www.fusetools.com/docs/fuse/controls/panel) 에서 작성 되었기 때문에, 실제로 `FontSize` 속성을 가지고 있지 않습니다. 그렇지만 별 문제가 되지는 않습니다, 우리가 그것을 스스로 만들 수 있습니다!
...
이제, 여기서 멈출 수도 있지만, 기억하십시오, 이 컴포넌트는 앱의 다른 곳에서 사용되며, 우리는 해당 인스턴스에 대한 `FontSize` 속성을 지정하지 않았습니다. 따라서 이 속성이 우리 컴포넌트에서 기본값을 가지고 있는지 확인해 봐야 하므로, 해당 인스턴스가 여전히 동일하게 보일 것입니다. 속성에 대한 기본값을 지정하려면, 인스턴스에 대해 설정하는 것처럼, 컴포넌트 정의에서 속성을 설정해야합니다. `hikr.Button.ux` 에서 이것은 다음과 같이 보입니다 :

```
<Panel ux:Class="hikr.Button" Margin="10" Padding="10" FontSize="16">
```

이런식으로, 지정하지 않으면, `FontSize` 속성의 값은 이전과 같이 16 이 될 것입니다. 멋집니다!

이제 버튼을 `HomePage` 로 이동하도록 합시다. 가장 먼저 할 일은 내비게이션을 수행 할 콜백을 포함하는 `SplashPage` 용 간단한 자바스크립트 모듈을 추가하는 것입니다. 그리고 우리는 우리 버튼의 Clicked 이벤트에 콜백을 바인딩 할 것입니다. 우리는 `Pages` 디렉토리 안에 `SplashPage.js` 라는 새로운 파일을 만들 것입니다. 그리고 내부에 `goToHomePage` 라는 함수를 만들 것입니다 :
...
이제 `SplashPage` 의 JS 모듈을 설정하고 버튼에 대한 `Clicked` 핸들러를 갖게 되었으므로, 이 핸들러를 제대로 구현하여 `HomePage` 로 이동 할 수 있습니다. 그런데 [Navigation and routing](https://www.fusetools.com/docs/tutorial/navigation-and-routing) 챕터를 생각해 보면, 네비게이션을 수행 하기 위해 하나 이상의 항목에 액세스 할 수 있도록 컴포넌트가 필요 할 겁니다. - 우리 앱의 [Router](https://www.fusetools.com/docs/fuse/navigation/router) 인스턴스 입니다!

먼저 `SplashPage` 에 `router` 라 불리는 `Router` 디펜던시를 추가합시다.
...
그런 다음, 다른 `Page` 들과 마찬가지로, `Router` 인스턴스를 전달함으로써 `MainView.ux` 에서 이러한 디펜던시를 충족시킬 수 있습니다.:
...
이제 `Router` 인스턴스를 사용하여 원하는 탐색을 수행 할 수 있습니다. `SplashPage.js` 로 돌아가서, `goToHomePage` 함수가 `home` 경로를 사용하는 `HomePage` 로 이동하도록 합시다.:
...
그렇게 하면, 모든 것을 저장하고 테스트 하여 작동하는지 확인할 수 있습니다! 그런데, 여기서는 `Router` 의 `Push` 함수를 사용하고 있다는 걸 유의하십시오. 이상적으로는, 정상적인 사용 중에 앱 사용자가 스플래시 화면으로 다시 돌아가는 것이 좋지 않으므로, `goto` 를 사용할 것 같습니다. 그런데 우리는 테스트가 더 쉽도록, 현재 `push` 를 사용하여 당분간 뒤로 이동할 수 있고, 나중에 `goto` 를 호출하는 것으로 교체 할 수 있습니다.
...
이것을 저장하고 Fuse가 연결된 모든 장치에 비디오 파일을 전송할 때까지 기다리면, 비디오 파일이 로드 되는 동안, 검은색 직사각형처럼 보입니다. 이는 `Video` 오브젝트가 기본적으로는 재생을 시작하지 않기 때문입니다. 우리는 실제 비디오를 재생하도록 지시해야 합니다. 이를 위해, `AutoPlay="true"` 로 설정합니다.:

```
    <Video Layer="Background" File="../Assets/nature.mp4" AutoPlay="true" />
```

이제 동영상이 재생되기 시작했습니다! 그런데 비디오의 끝까지 도달하면 재생이 멈추는 것을 볼 수 있습니다. 동영상을 반복하고 싶기 때문에 `IsLooping="true"` 로 설정합니다.

```
    <Video Layer="Background" File="../Assets/nature.mp4" IsLooping="true" AutoPlay="true" />
```

다음으로 해결해야 할 사항은, 디바이스/미리보기 가 동영상과 다른 가로 세로 비율을 가지고 있다면, 동영상이 전체 화면을 채우지 않을 것입니다. [이전 챕터](https://www.fusetools.com/docs/tutorial/look-and-feel) 의 배경 이미지와 유사하게, 이는 `Video` 요소가 항상 사용 가능한 공간에 맞춰지기 위해 내용을 늘리거나 찌그러 뜨려 가로 세로 비율을 유지하기 때문입니다. 우리는 항상 비디오가 사용 가능한 공간을 채우길 원하기 때문에 `hikr.Page` 컴포넌트가 배경 이미지와 마찬가지로 `StretchMode="UniformToFill"` 을 사용합니다.:

```
    <Video Layer="Background" File="../Assets/nature.mp4" IsLooping="true" AutoPlay="true" StretchMode="UniformToFill" />
```

이제 좋아 보입니다! 그런데, 우리의 배경은 그대로 있는 것이 너무 뚜렷하기 때문에, 약간의 흐림 효과를 추가하여 "되돌려" 봅시다. `Video` (또는 실제로 Fuse의 모든 요소)를 흐리게 처리하려면 [Blur](https://www.fusetools.com/docs/fuse/effects/blur) 요소를 추가해야 합니다.:
...
이것으로 우리는 `Video` 가 멋지게 흐림 처리된 것을 볼 수 있습니다! 얼마나 흐릿하게 조정할 것인지 `Radius` 를 약간 조정 해 봅시다. 우리는 모양을 유지하면서 상당히 흐리게 보이기를 원합니다.:
...
수평 중앙으로 정렬하고 약간의 여백을 추가하여, 하단 가장자리를 완전히 터치하지 않도록 합시다.:
...
그리고 그것으로 우리는 우리의 컨텐츠를 적절하게 만들었고, 멋지게 보입니다!

## 마감 처리 ##

이제 `SplashScreen` 이 기본적으로 완료 되었으므로, 앱에 몇 가지 마무리 터치를 추가하여 완료 할 수 있습니다. 예를 들어, `HomePage` 로 이동하여 화면의 왼쪽 가장자리를 보면, `SplashScreen` 에서 [Video](https://www.fusetools.com/docs/fuse/controls/video) 의 작은 부분을 볼 수 있습니다. 이는 [Blur](https://www.fusetools.com/docs/fuse/effects/blur) 효과가 적용된 요소 주위에 약간의 공간을 추가하여, 요소의 가장자리가 부드럽게 흐려지기 때문입니다. 그러나 우리의 경우, 이는 앱의 다른 페이지에서 동영상 가장자리의 일부를 볼 수 있음을 의미하므로 바람직하지 않습니다. 이 문제를 해결하려면, `SplashPage` 의 내용이 페이지의 경계에 잘려져 있는지 확인해야 합니다. 우리의 경우 `SplashPage.ux` 내 [DockPanel](https://www.fusetools.com/docs/fuse/controls/dockpanel) ( `Video` 배경 포함 ) 에 `ClipToBounds="true"` 를 간단히 추가하면 됩니다.
...
이제 동영상은 `SplashPage` 경계 내에서만 표시됩니다. 멋집니다!
...
우리가 다 만든 것 같습니다! 우리 hikr 컨셉 앱은 완전히 완료되었습니다. 훌륭합니다!
...
이제 앱이 끝났으므로, Fuse의 핵심 컨셉들을 이미 많이 다뤘지만, 항상 배울 것은 많습니다. [마지막 챕터](https://www.fusetools.com/docs/tutorial/final-thoughts) 는 우리가 배웠던 것에 대해, 그리고 Fuse 스킬들을 향상시키기 위해 다음으로 갈 수 있는 부분에 대해, 빠르게 마무리 할 것입니다. 그럼 그걸 [확인해 봅시다](https://www.fusetools.com/docs/tutorial/final-thoughts) !
