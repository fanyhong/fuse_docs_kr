...
이 UX 컴포넌트는 크로스 플랫폼 앱 화면들을 만들기 위한 기본 컴포넌트들입니다. 이러한 컴포넌트들은 `Native` 모드와 `Graphics` 모드 모두에서, 모든 테마들이 작동합니다. 특정 컨트롤(버튼과 같은) 의 look and feel(모양과 느낌) 은 테마에 따라 다를 수 있습니다.
